...
<section class="mt-16">
	<h2 class="mb-8 text-center text-2xl font-bold">
		{language.currentLanguage === 'ja' ? '企業からの声' : 'What Our Clients Say'}
	</h2>
		...
		{#each testimonials as testimonial}
			<div class="rounded-lg bg-white p-6 shadow-md dark:bg-gray-800">
				<div class="mb-4 flex items-center">
					<img
						alt={language.currentLanguage === 'ja' ? testimonial.name.ja : testimonial.name.en}
						class="mr-4 h-12 w-12 rounded-full object-cover"
						onerror={handleImageError}
					/>
					<div>
						<h3 class="font-medium">
							{language.currentLanguage === 'ja' ? testimonial.name.ja : testimonial.name.en}
						</h3>
						<p class="text-sm text-gray-600 dark:text-gray-400">
							{language.currentLanguage === 'ja' ? testimonial.role.ja : testimonial.role.en},
							{language.currentLanguage === 'ja' ? testimonial.company.ja : testimonial.company.en}
						</p>
					</div>
				</div>
				<p class="text-gray-700 italic dark:text-gray-300">
					"{language.currentLanguage === 'ja' ? testimonial.text.ja : testimonial.text.en}"
				</p>
			</div>
		{/each}
	</div>
